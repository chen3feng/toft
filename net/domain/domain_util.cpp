        ...
        if (rule && (private_filter == INCLUDE_PRIVATE_REGISTRIES || !rule->is_private)
            ...
            if (rule->type == kExceptionRule) {
                if (next_dot == std::string::npos) {
                    ...
                    DCHECK(false) << "Invalid exception rule";
                    return 0;
                }
...
std::string GetDomainAndRegistryImpl(const std::string& host,
    ...
    if (registry_length > (host.length() - 2)) {
        DCHECK(false) << "Host does not have at least one subcomponent before registry!";
        return std::string();
    }
...
static bool HostIsIPAddress(const std::string& host) {
    // FIXME(yeshunping) : support ipv6, implement this function in URI class
    int dot_num = 0;
...
std::string DomainUtil::GetDomainAndRegistry(const URI& uri, PrivateRegistryFilter filter) {
...
std::string DomainUtil::GetDomainAndRegistry(const std::string& host,
...
bool DomainUtil::SameDomainOrHost(const URI& gurl1, const URI& gurl2,
                                  PrivateRegistryFilter filter) {
...
size_t DomainUtil::GetRegistryLength(const URI& uri,
...
size_t DomainUtil::GetRegistryLength(const std::string& host,
                                     UnknownRegistryFilter unknown_filter,
                                     PrivateRegistryFilter private_filter) {
    if (host.empty())
        return std::string::npos;

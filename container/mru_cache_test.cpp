...
#include "toft/container/mru_cache.h"
...
TEST(MruCacheTest, Basic) {
    typedef toft::MruCache<int, CachedItem> Cache;
...
TEST(MruCacheTest, Owning) {
    typedef toft::OwningMruCache<int, CachedItem*> Cache;
    Cache cache(Cache::NO_AUTO_EVICT);
...
TEST(MruCacheTest, HashingMruCache) {
    ...
    typedef toft::HashingMruCache<std::string, CachedItem> Cache;
    Cache cache(Cache::NO_AUTO_EVICT);

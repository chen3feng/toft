...
toft::URI GetURI(const std::string& url) {
    toft::URI uri;
    uri.Parse(url);
    return uri;
}
...
std::string GetDomainFromURL(const std::string& url) {
    return GetDomainAndRegistry(GetURI(url), EXCLUDE_PRIVATE_REGISTRIES);
}
...
size_t GetRegistryLengthFromURL(
    ...
    return GetRegistryLength(GetURI(url),
                             unknown_filter,
...
bool CompareDomains(const std::string& url1, const std::string& url2) {
    toft::URI g1 = GetURI(url1);
    toft::URI g2 = GetURI(url2);
...
TEST_F(RegistryControlledDomainTest, TestGetDomainAndRegistry) {
    ...
    // Test toft::URI version of GetDomainAndRegistry().
...
//  EXPECT_EQ("baz.com.", GetDomainFromURL("http://baz.com."));            // none
    ...
    // underpinnings as the toft::URI version, so this is really more of a check of
    // CanonicalizeHost().
...
TEST_F(RegistryControlledDomainTest, TestGetRegistryLength) {
    ...
    // Test toft::URI version of GetRegistryLength().
